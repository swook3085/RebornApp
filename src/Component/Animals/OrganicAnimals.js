...
class OrganicAnimals extends Component {
  ...
  getKindInfoApi = async upkind => {
    ...
    if (this.state.upkind != 0) {
      await axios
        .get(`${SERVICE_URL}kind?up_kind_cd=${upkind}&serviceKey=${SERVICE_KEY}`)
        .then(res => {
          ...
          if (
            ...
            if (Array.isArray(res.data.response.body.items.item)) {
              ...
              kindData.push({
                label: res.data.response.body.items.item.KNm,
                value: res.data.response.body.items.item.kindCd,
              });
            }
            this.setState({
  ...
  getSigunguInfoApi = async orgCd => {
    ...
    if (this.state.sido != 0) {
      await axios
        .get(`${SERVICE_URL}sigungu?upr_cd=${orgCd}&serviceKey=${SERVICE_KEY}`)
        .then(res => {
          console.log(res.data.response.body);
      ...
      <>
        <View style={styles.panel}>
          <View style={{ height: 200, paddingHorizontal: 20 }}>
            ...
            <View style={{ flex: 1, flexDirection: 'row' }}>
              <Animatable.View
                style={{ flex: 1, height: 150, padding: 10 }}
                animation="fadeIn"
                delay={1 * 200}
              >
                <TouchableOpacity
                  activeOpacity={1}
                  style={[styles.card, this.state.upkind == 0 && styles.active]}
                  onPress={() => this.inputOnChange('upkind', '0')}
                >
              ...
              >
                <TouchableOpacity
                  activeOpacity={1}
                  style={[styles.card, this.state.upkind == 417000 && styles.active]}
                  onPress={() => this.inputOnChange('upkind', '417000')}
                >
                  <View style={{ height: 60, width: 60 }}>
                    <Image source={dog} resizeMode={'contain'} style={{ width: 60, height: 60 }} />
                  </View>
              ...
              >
                <TouchableOpacity
                  activeOpacity={1}
                  style={[styles.card, this.state.upkind == 422400 && styles.active]}
                  onPress={() => this.inputOnChange('upkind', '422400')}
                >
                  <View style={{ height: 60, width: 60 }}>
                ...
                {this.state.sidoData.map((data, index) => {
                  return (
                    <TouchableOpacity
                      ...
                      onPress={() => this.inputOnChange('sido', data.value)}
                    >
                      <Animatable.View
                        style={[
                          styles.card,
                          this.state.sido == data.value && styles.sidoActive,
                          { padding: 10 },
                        ]}
                        animation="fadeInRight"
                        delay={index * 100}
                      >
                        <Text style={this.state.sido == data.value && styles.sidoTextActive}>
                          {data.label}
                        </Text>
                      </Animatable.View>
                    </TouchableOpacity>
            ...
            <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
              <Animatable.View style={{ flex: 1 }} animation="fadeInUp" delay={200}>
                <TouchableOpacity
                  ...
                  activeOpacity={1}
                  onPress={() => this.RBSheetStartDate.open()}
                >
                  <Text style={{ fontSize: 16 }}>
                    {dateFomat(dateToString(this.state.startDate))}
                  </Text>
                </TouchableOpacity>
              </Animatable.View>
              <Text>~</Text>
              ...
              >
                <TouchableOpacity
                  ...
                  activeOpacity={1}
                  onPress={() => this.RBSheetEndDate.open()}
                >
                  <Text style={{ fontSize: 16 }}>
                    {dateFomat(dateToString(this.state.endDate))}
                  </Text>
                </TouchableOpacity>
              </Animatable.View>
            </View>
          ...
          <TouchableOpacity
            ...
            activeOpacity={1}
            onPress={this.searchOnPressTest}
          >
        ...
        >
          ...
          <TouchableOpacity
            ...
            activeOpacity={1}
            onPress={this.onDateEvent('startDate')}
          >
            <Text style={styles.doneText}>설정</Text>
          </TouchableOpacity>
        </RBSheet>
        <RBSheet
          ref={ref => {
            this.RBSheetEndDate = ref;
          }}
          height={330}
          duration={250}
          customStyles={{
        ...
        >
          ...
          <TouchableOpacity
            ...
            activeOpacity={1}
            onPress={this.onDateEvent('endDate')}
          >
            <Text style={styles.doneText}>설정</Text>
          </TouchableOpacity>
        </RBSheet>
        ...
        <Animated.View style={[styles.container]}>
          ...
          <Animated.View
            style={[
              ...
              { height: this.state.expansionAnimate },
            ]}
          >
            <View style={{ height: 40, flex: 1 }}>
              ...
              >
                ...
                >
                  ...
                  >
                    {convertUpkind(this.state.upkind)}
                  </Text>
                  <Text
                    style={{
                      backgroundColor: '#ECB04D',
                  ...
                  >
                    {convertSido(this.state.sido.toString())}
                  </Text>
                </View>
              </View>
            </View>
